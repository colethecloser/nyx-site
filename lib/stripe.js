...
import { COHORT_PRICE_CENTS, billingPeriod, optionalEnv, requireEnv, siteUrl } from './env.js';
...
export function getStripe() {
  if (!globalForStripe.__stripe) {
    ...
    globalForStripe.__stripe = new Stripe(key, {
      ...
      appInfo: { name: 'FGCU Student Investment Group', version: '1.0.0' },
      maxNetworkRetries: 2,
    });
  }
...
/**
 * A configured Price wins; otherwise the dues price is described inline so the
 * platform runs against a bare Stripe account with no dashboard setup.
 */
function lineItem() {
  ...
  return {
    ...
    price_data: {
      ...
      recurring: { interval: period.interval, interval_count: period.intervalCount },
      product_data: {
        name: `FGCU Student Investment Group — club dues (${period.noun})`,
        description:
          'Weekly deliverable review, the analyst leaderboard, and every session in the trading room.',
      },
    },
  };
}
