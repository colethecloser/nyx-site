import { billingPeriod, siteUrl } from '../env.js';
...
export function applicationAccepted({ name, cohortName, inviteUrl, priceLabel, seatsLeft, expiresLabel }) {
  const lines = [
    ...
    `Club dues are ${priceLabel} ${billingPeriod().label}. They cover weekly review of your work, the analyst leaderboard, and every session in the trading room.`,
...
export function paymentFailed({ name, attemptLabel, billingUrl }) {
  const lines = [
    `${name} — your dues payment did not go through.`,
  ...
  return {
    subject: 'Payment issue on your dues',
    html: layout({
      preheader: 'Your dues payment failed. Update your card.',
...
export function subscriptionEnded({ name, cohortName }) {
  const lines = [
    `${name} — your ${cohortName} membership has ended and dues have stopped.`,
...
export function renewalConfirmed({ name, amountLabel, periodEndLabel }) {
  const lines = [
    `${name} — your dues renewed for ${amountLabel}.`,
    `You are paid through ${periodEndLabel}.`,
  ];
  return {
    subject: 'Dues renewed',
    html: layout({
      preheader: 'Your club dues renewed.',
      heading: 'Dues renewed',
      blocks: lines.map(p),
    }),
    text: textify(lines),
  };
