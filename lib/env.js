...
export const COHORT_PRICE_CENTS = intEnv('COHORT_PRICE_CENTS', 179900);

/**
 * How often dues are charged.
 *
 * Stripe has no "semester" interval, so a semester is billed as every six
 * months — the closest recurring shape it supports, and the one that keeps the
 * whole renewal, dunning and cancellation machinery working unchanged. Set
 * COHORT_BILLING_PERIOD to `semester`, `year` or `month`.
 */
const BILLING_PERIODS = {
  semester: { label: 'per semester', noun: 'semester', interval: 'month', intervalCount: 6 },
  year: { label: 'per year', noun: 'year', interval: 'year', intervalCount: 1 },
  month: { label: 'per month', noun: 'month', interval: 'month', intervalCount: 1 },
};

export function billingPeriod() {
  const key = optionalEnv('COHORT_BILLING_PERIOD', 'semester');
  return BILLING_PERIODS[key] ?? BILLING_PERIODS.semester;
}

/**
 * Set COHORT_PRICE_CENTS=0 to run the group with no dues at all.
 *
 * The public page then makes no claim about money, and an accepted applicant is
 * enrolled directly instead of being sent to Stripe — which matters because
 * Stripe rejects a zero-amount subscription, so "free" has to be a real branch
 * rather than a price of nothing.
 */
export function isFree() {
  return COHORT_PRICE_CENTS <= 0;
}
...
export const PROGRAM = {
  name: optionalEnv('COHORT_NAME', 'FGCU Student Investment Group'),
